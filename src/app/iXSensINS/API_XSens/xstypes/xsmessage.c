...
static inline void XsMessage_getEndianCorrectData(XsMessage const* thisPtr, void *value, XsSize size, XsSize offset)
{
...
static inline void XsMessage_setEndianCorrectData(XsMessage *thisPtr, void *value, XsSize size, XsSize offset)
{
	XsMessage_ensureDataSize(thisPtr, offset, size);
	XsMessage_updateChecksumWithValue(thisPtr, value, size, offset);
	swapEndian(value, size);
	memcpy(XsMessage_dataAtOffset(thisPtr, offset), value, size);
}
