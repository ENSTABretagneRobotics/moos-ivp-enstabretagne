...
void nmea_PRTI012info(const nmeaPRTI01 *pack, nmeaINFO *info) {
	assert(pack);
	assert(info);

	if (nmea_INFO_is_present(pack->present, RTI)) {
		info->rti_utc = pack->utc;
		info->rti_sample_nb = pack->sample_nb;
		info->rti_temp = pack->temp;
		info->rti_bottom_vx = pack->bottom_vx;
		info->rti_bottom_vy = pack->bottom_vy;
		info->rti_bottom_vz = pack->bottom_vz;
		info->rti_bottom_depth = pack->bottom_depth;
		info->rti_water_vx = pack->water_vx;
		info->rti_water_vy = pack->water_vy;
		info->rti_water_vz = pack->water_vz;
		info->rti_water_depth = pack->water_depth;
		strcpy(info->rti_cbit, pack->cbit);
		info->rti_S = pack->S;
		info->rti_N = pack->N;
	}
}
