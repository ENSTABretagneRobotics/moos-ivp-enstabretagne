  ...
  for (charIndex = 0; charIndex < len; charIndex++) {
    ...
    if (sentence_read_successfully) {
      ...
      switch (sentence_type) {
        ...
        case PRTI01:
          if (nmea_parse_PRTI01(parser->buffer.buffer, parser->buffer.length, parser->sentence_parser.has_checksum, &parser->sentence.prti01)) {
            sentences_count++;
            nmea_PRTI012info(&parser->sentence.prti01, info);
          }
          break;
